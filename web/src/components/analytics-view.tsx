...
const STAGE_KEYS = ["EVALUATED", "APPLIED", "RESPONDED", "INTERVIEW", "OFFER", "HIRED", "REJECTED", "DISCARDED"] as const;
...
export function AnalyticsView({ applications }: { applications: Application[] }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      ...
      <p className="mt-1 text-sm text-muted">{t("analytics.across", { n: total })}</p>

      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <Stat value={total} label={t("analytics.evaluated")} />
        <Stat value={avg ? avg.toFixed(2) : "—"} label={t("analytics.avg")} />
        <Stat
          value={interviews}
          label={t("analytics.interviews")}
          hint={interviews === 0 ? t("analytics.hintInterview") : undefined}
        />
        <Stat
          value={offers}
          label={t("analytics.offers")}
          hint={offers === 0 ? t("analytics.hintOffer") : undefined}
        />
      </div>
      ...
      <Section title={t("analytics.byStage")}>
        {stageCounts.map((s) => (
          <Bar
            key={s.key}
            label={t(`status.${s.key}` as MsgKey, s.key)}
            value={s.n}
      ...
      <Section title={t("analytics.byScore")}>
        {buckets.map((b) => (
