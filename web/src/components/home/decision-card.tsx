  ...
  return (
    <div className="flex min-w-0 flex-col gap-2.5 rounded-xl border border-border bg-surface/40 p-3.5 transition hover:border-brand/30">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {busy === "Applied" ? <Loader2 className="size-3.5 animate-spin" /> : <Check className="size-3.5" />} {t("decision.apply")}
        </button>
        <button
          type="button"
          disabled={!!busy}
        ...
        <a href={`/pipeline/${app.n}`} title={t("decision.report")} aria-label={t("decision.report")} className="inline-flex shrink-0 items-center justify-center rounded p-1.5 text-faint transition hover:text-brand max-sm:min-h-[44px] max-sm:min-w-[44px]">
