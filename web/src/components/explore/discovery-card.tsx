...
import type { MsgKey, MsgVars } from "@/lib/i18n/messages";

function freshness(postedAt: string, t: (key: MsgKey, vars?: MsgVars) => string): string {
  ...
  return days === 0 ? t("card.today") : days === 1 ? t("card.ago1") : t("card.agoN", { n: days });
}
...
export function DiscoveryCard({ offer, inPipeline, evaluatedN }: { offer: DiscoveredOffer; inPipeline: boolean; evaluatedN?: string }) {
  ...
  const WORKER_LABEL: Record<string, string> = {
    evaluate: t("card.evaling"),
    pdf: t("card.pdf"),
    research: t("card.research"),
    apply: t("card.apply"),
  };
  const statusLabel = WORKER_LABEL[job?.kind ?? ""] ?? t("card.working");
  ...
  const evaluate = () => {
    ...
    startJob({ title: `Evaluate · ${offer.company}`, subtitle: offer.title, kind: "evaluate", input: offer.url, page: "/evaluate" });
    ...
    <div className="co-rise group flex min-w-0 flex-col gap-2.5 rounded-xl border border-border bg-surface/40 p-3.5 text-left transition-all hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <a
          ...
          title={t("card.openPosting")}
          aria-label={t("card.openPosting")}
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
        ...
        {unverified && (
          ...
          >
            <ShieldQuestion className="size-3" /> {t("card.unverified")}
          </span>
        )}
        {offer.matchedKeyword && (
          <span className="text-faint" title="Keyword match — not yet scored. Evaluate to get an A–F fit score.">
            {t("card.matched")} <span className="text-brand/80">{offer.matchedKeyword}</span>
          </span>
        )}
      </div>
      ...
      <div className="mt-0.5">
        {evaluatedN || doneEval ? (
          ...
          >
            <Check className="size-3.5" /> {t("card.evaluated")}
          </a>
        ) : working ? (
          <a
            href="/evaluate"
            className="inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-brand/30 bg-brand-soft/60 px-2.5 py-2 text-xs font-medium text-brand max-sm:min-h-[44px]"
          >
            <Loader2 className="size-3.5 animate-spin" />
            {statusLabel}
            <span className="text-brand/60">{t("card.watch")}</span>
          </a>
        ) : (
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {isAdded ? t("card.inPipeline") : t("card.addPipeline")}
            </button>
            <button
              type="button"
              onClick={evaluate}
            ...
            >
              {t("card.evaluate")} <Coins className="size-3.5 opacity-80" />
            </button>
          </div>
        )}
      </div>
