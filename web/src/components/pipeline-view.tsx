      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            {t("pipeline.counts", { inbox: pendingInbox.length, tracked: applications.length })}
          </p>
        </div>
        ...
        {TABS.map((tabId) => {
          const count =
            tabId === "INBOX"
              ...
              : tabId === "ALL"
                ? applications.length
                : applications.filter((r) => canonStatus(r.status).includes(tabId)).length;
          return (
            <button
              key={tabId}
              onClick={() => setParams({ tab: tabId === "INBOX" ? null : tabId })}
              className={cn(
            ...
            >
              {t(`status.${tabId}` as MsgKey, tabId)} <span className="text-faint tabular-nums">{count}</span>
            </button>
          );
        })}
      </div>
      ...
      {tab !== "INBOX" && minFilter != null && (
        <div className="mt-3 flex items-center gap-2">
          ...
          <button
            ...
            title={t("pipeline.clearScore")}
          >
            {t("pipeline.scoreGte", { n: minFilter.toFixed(1) })}
            <X className="size-3" />
          </button>
        </div>
      )}
                ...
                {SORT_KEYS.map((k) => (
                  ...
                  >
                    <span className="inline-flex items-center gap-1">
                      {k === "date" ? t("pipeline.col.date") : t(`followups.col.${k}` as MsgKey, k)}
  ...
  if (filtered) {
    return (
      <div className="mt-4 rounded-2xl border border-dashed border-border bg-surface/30 px-6 py-12 text-center">
        ...
        <p className="mx-auto mt-1 max-w-sm text-sm text-muted">{t("inbox.clearSearch")}</p>
      </div>
    );
  }
  return (
    <div className="dot-bg mt-4 overflow-hidden rounded-2xl border border-border bg-surface/50 bg-origin-border bg-gradient-to-tr from-brand/10 via-transparent to-transparent shadow-lg">
      ...
      <div className="px-6 py-10 text-center">
        <p className="font-display text-lg">
          {t("inbox.emptyTitleBefore")} <span className="text-brand">{t("inbox.emptyTitleEm")}</span> {t("inbox.emptyTitleAfter")}
        </p>
        {count > 0 ? (
          <p className="mx-auto mt-2 max-w-sm text-sm text-muted">{t("inbox.emptyPending")}</p>
        ) : (
          <>
            <p className="mx-auto mt-2 max-w-sm text-sm text-muted">{t("inbox.emptyLead")}</p>
            <Link
              href="/explore?run=1"
            ...
            >
              <Compass className="size-4" /> {t("inbox.firstScan")} <ArrowRight className="size-4" />
            </Link>
